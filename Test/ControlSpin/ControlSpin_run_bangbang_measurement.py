...
paramsSim = {'setup' : setup, 'target_state_name' : state_target, 'init_state_name':state_init, 
             ...
             'T': T, 'fom_name': 'last:proj100:neg:rdmtime_fluenceNorm:0.0001_smooth:0.0005',
             'noise': noise}
...
res_optimNM = modelTestNM.Simulate(T, time_step = dt, method = 'testing', fom = ['last:f2t2:rdmtime', 'fluence', 'fluenceNorm', 'smooth'], store = True)
...
paramsOptim = {'algo': 'DE', 'name_res': None, 'nb_params' :  nb_parameters, 
               'params_init':'uniform_0_1', 'params_bound':'range_0_1', 'de_maxiter':100}
...
modelTestDE = cspin.ControlledSpin(cfBestDE, setup, state_init, state_target, noise = noise_testing)
res_optimDE = modelTestDE.Simulate(T, time_step = dt, method = 'PWCH', fom = ['last:f2t2:rdmtime', 'fluence', 'fluenceNorm'], store = True)
...
paramsOptim = {'algo': 'GP', 'name_res': None, 'nb_params' :  nb_parameters, 
               ...
               'gp_wnoise': 0.0001, 'gp_maxiter':100}
...
if(False):
    ...
    test = optimNM.resOptim['func_fom'][0][0]
    # Pick the best then compare to all the stuff over time
    list_funcNM = [pfuncollec.Repr2Fun(r) for r, _ in optimNM.resOptim['func_fom']]
    list_funcDE = [pfuncollec.Repr2Fun(r) for r, _ in optimDE.resOptim['func_fom']]
    list_funcBO = [pfuncollec.Repr2Fun(r) for r, _ in optimBO.resOptim['func_fom']]
    
    
    
    #Compute the ideal (no noise FOM)
    def computeFOMIdeal(func):
        modelTmp = cspin.ControlledSpin(func, setup, state_init, state_target, noise = None)
        resTmp = modelTmp.Simulate(T, time_step = dt, method = 'PWCH', fom = ['last:f2t2'], store = False)
        return resTmp
    list_fom_idealNM = [computeFOMIdeal(f)[0] for f in list_funcNM]
    list_fom_idealDE = [computeFOMIdeal(f)[0] for f in list_funcDE]
    list_fom_idealBO = [computeFOMIdeal(f)[0] for f in list_funcBO]
    list_fom_ideal = [list_fom_idealNM, list_fom_idealDE, list_fom_idealBO]
    ...
    for i in range(len(name)):
        ax.plot(list_iter[i], list_fom_ideal[i], label=name[i])
    ax.legend()
    plt.xlim([0,200])
    plt.ylim([0, 0.1])
