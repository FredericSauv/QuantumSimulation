...
ax2.set_yticks([])
ax2.set_yticks([0., 2.])
ax1.text(-0.4, 1.1, '(a)', fontsize=font_size)
ax1.scatter(m_classi.X[-1], m_classi.Y[-1]/nb_meas, label = r'$New\;Obs$', marker = 's', c='g', s=100)
print(m_classi)
ax2.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
ax2.set_yticks([0.00, 2.00])
ax1.set_ylabel('Landscape', fontsize=font_size)
ax2.set_ylabel('Acqu.', fontsize=font_size)
...
if (save):
    ax2.text(0.7, -1, r'Control parameter $\theta$', fontsize=font_size)
...
for n in range(n_iter):
    ...
    y_next = counts(x_next, underlying_p = proba2, nb_measures=nb_meas)
    X_new = np.vstack([m_classi.X, x_next])
    Y_new = np.vstack([m_classi.Y, y_next])
    m_classi.Y_metadata = {'trials':np.ones_like(Y_new)*nb_meas}
...
ax1.plot(x_test, c, color = col_custom, alpha = 0.5, linewidth = 0.4)
ax1.imshow(np.power(d[np.arange(len(d)-1,-1, -1)]/np.max(d), 0.75), cmap = 'Blues', aspect='auto', interpolation='spline16', extent = (x_range[0],  x_range[1], d_range[0], d_range[1]), alpha=1)
ax1.scatter(m_classi.X, m_classi.Y/nb_meas, label = r'$Observations$', marker = 'o', c='red', s=points_size)
ax1.plot(x_test, p_test, 'r--', label='F')
ax1.scatter(m_classi.X[-1], m_classi.Y[-1]/nb_meas, label = r'$New\;Obs$', marker = 's', c='g', s=100)
acq = (a + weight * s)
ax2.plot(x_test, acq,color='r')
...
ax2.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
ax2.set_yticks([0.00, 2.00])
ax1.text(-0.4, 1.1, '(b)', fontsize=font_size)

x_best = next_ucb_std_v2(m_classi, m_classi.X, 0)
pres = proba2(x_best)
ax1.vlines(x_best, -0.1, 1, colors='g', label = r'$\theta_{best}$')
print(pres)
...
ax1.plot(x_test, c, color = col_custom, alpha = 0.5, linewidth = 0.4)
ax1.imshow(np.power(d[np.arange(len(d)-1,-1, -1)]/np.max(d), 0.75), cmap = 'Blues', aspect='auto', interpolation='spline16', extent = (x_range[0],  x_range[1], d_range[0], d_range[1]), alpha=1)
ax1.scatter(m_classi.X, m_classi.Y, label = r'$Observations$', marker = 'o', c='red', s=points_size)
ax1.plot(x_test, p_test, 'r--', label='F')
ax1.scatter(m_classi.X[-1], m_classi.Y[-1], label = r'$New\;Obs$', marker = 's', c='g', s=100)
acq = (a + weight * s)
ax2.plot(x_test, acq,color='r')
x_next = next_ucb_std_v2(m_classi, x_test, weight)
...
ax2.yaxis.set_major_formatter(FormatStrFormatter('%.1f'))
ax2.set_yticks([0.00, 2.00])
ax1.text(-0.4, 1.1, '(c)', fontsize=20)
#ax2.set_ylabel(r'$ a.u.$', fontsize=16)

pres = proba2(x_best)
if (save):
    plt.savefig(version+"init91_nothet"+str(pres)+".pdf", bbox_inches='tight', transparent=True, pad_inches=0)
ax2.text(0.7, -1., r'Control parameter $\theta$', fontsize=20)
if (save):
    plt.savefig(version+"init91.pdf", bbox_inches='tight', transparent=True, pad_inches=0)
...
ax2.vlines(x_next, -0.01, np.max(acq), colors='r', linestyles='dashed')
ax1.set_ylim([-0.05, 1.05])
ax1.set_xlim([-0.02, 4.02])
ax2.set_xlim([-0.02, 4.02])
ax2.xaxis.tick_top()
...
ax1.plot(x_test, p_test, 'r--', label='F')
ax1.scatter(m_classi.X[-1], m_classi.Y[-1], label = r'$New\;Obs$', marker = 's', c='g', s=points_size)
acq = (a + weight * s)
ax2.plot(x_test, acq,color='r')
x_next = next_ucb_std_v2(m_classi, x_test, weight)
ax2.vlines(x_next, np.min(acq), np.max(acq), colors='r', linestyles='dashed')
