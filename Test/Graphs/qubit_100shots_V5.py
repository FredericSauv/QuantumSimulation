...
plt.imshow(np.power(d[np.arange(len(d)-1,-1, -1)]/np.max(d), 1), cmap = 'Blues', aspect='auto', interpolation='spline16', extent = (x_range[0],  x_range[1], d_range[0], d_range[1]), alpha=1)
...
plt.scatter(x_next, 0, marker = '*', c='r', s=80)
plt.vlines(x_next, -3,0, colors='r', linestyles='dashed')
plt.ylim([-0.3, 1.02])
plt.xlim([-0.03, 4.])
...
plt.scatter(x_init, y_init, label = r'$Observations$', marker = 'o', c='red', s=25)
plt.plot(x_test, p_test, 'r--', label='F')
plt.scatter(m_classi.X[-1], m_classi.Y[-1], label = r'$New\;Obs$', marker = 's', c='g', s=80)
...
plt.plot(x_test, c, color = col_custom, alpha = 0.5, linewidth = 0.4)
plt.imshow(np.power(d[np.arange(len(d)-1,-1, -1)]/np.max(d), 1), cmap = 'Blues', aspect='auto', interpolation='spline16', extent = (x_range[0],  x_range[1], d_range[0], d_range[1]), alpha=1)
plt.scatter(m_classi.X, m_classi.Y, label = r'$Observations$', marker = 'o', c='red', s=25)
plt.plot(x_test, p_test, 'r--', label='F')
plt.plot(x_test, p_test, 'r--')
acq = (a + weight * s)
acq_scale = (np.max(acq) - np.min(acq))
