...
def invcdf(y):
    return np.sqrt(2) * spe.erfinv(2*y-1)

def changedistrib(y, mu, var):
    inv = invcdf(y)
    res = stats.norm.pdf(inv, loc=mu, scale=var) / stats.norm.pdf(inv)
    res[np.isnan(res)]=0
    return  res
    
def predict_p_v2(model, X):
    mean, var = model._raw_predict(X, full_cov=False, kern=None)
    #likelihood = model.likelihood
    #Nf_samp = 10000
    #s = np.random.randn(mean.shape[0], Nf_samp) * np.sqrt(var) + mean
    ...
    density = np.array([changedistrib(y, m, np.sqrt(v)) for m, v in zip(mean, var)])
    p_min, p_max = 0, 1
...
def next_ucb_bounds_v2(model, loc, w=4):
    a, b, c, d, d_range, s = predict_p_v2(model, loc)
    acq = (a + w * (c-b))
    x_next = loc[np.argmax(acq)]
    return x_next

def next_ucb_std_v2(model, loc, w=4):
    a, b, c, d, d_range, s = predict_p_v2(model, loc)
    acq = (a + w * s)
    x_next = loc[np.argmax(acq)]
    return x_next
